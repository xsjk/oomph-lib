...
\hypertarget{index}{}\label{index}\index{Demo problem: Fluid Mechanics on unstructured 3D meshes@{Demo problem: Fluid Mechanics on unstructured 3D meshes}}
...
The specific problem considered here also serves as a "{}warm-\/up problem"{} for the \href{../../../interaction/unstructured_three_d_fsi/html/index.html}{\texttt{ corresponding fluid-\/structure interaction problem}} in which the domain boundary is replaced by an elastic vessel.
...
The 15 facets are defined in a {\ttfamily \texorpdfstring{$\ast$}{*}.poly} file that specifies the position of the vertices, and identifies the facets via a "{}face list"{} that establishes their bounding vertices. The well-\/annotated {\ttfamily \texorpdfstring{$\ast$}{*}.poly} file for this problem is located at\+:
...
We refer to the \href{http://wias-berlin.de/software/tetgen//}{\texttt{ {\ttfamily tetgen} webpages }} and {\ttfamily oomph-\/lib\textquotesingle{}s} own \href{../../../meshes/mesh_from_tetgen/html/index.html}{\texttt{ tetgen tutorial}} for further details on how to create {\ttfamily \texorpdfstring{$\ast$}{*}.poly} files.
...
\href{http://wias-berlin.de/software/tetgen//}{\texttt{ {\ttfamily Tetgen} }} generates an unstructured volumetric mesh from the information contained in the {\ttfamily \texorpdfstring{$\ast$}{*}.poly} file and outputs the mesh\textquotesingle{}s nodes, elements and faces in the files
\begin{DoxyItemize}
...
Note how \href{http://wias-berlin.de/software/tetgen//}{\texttt{ {\ttfamily tetgen} }} has subdivided each of the 15 original facets specified in the {\ttfamily \texorpdfstring{$\ast$}{*}.poly} file into a surface triangulation. The nodes and tetrahedral elements that are located on (or adjacent to) the 15 original facets inherit their boundary IDs. This is important when we assign the boundary conditions for the actual computation.

\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_results}{}\doxysection{\texorpdfstring{Results}{Results}}\label{index_results}
The plot shown below illustrates the flow field (streamribbons coloured by pressure contours) for a Reynolds number of $ Re = 100. $ The transparent faces show the boundaries of the fluid elements and illustrate that the mesh is very coarse. As a result, the flow is clearly under-\/resolved, particularly near the two outflow cross-\/sections where the imposition of parallel outflow forces the fluid velocity to re-\/adjust rapidly as it approaches the outlet. (See \doxysectlink{index_finer_mesh}{Creating a finer mesh and applying more appropriate outflow boundary conditions}{2} in \doxysectlink{index_comm_ex}{Comments and Exercises}{1} for a more detailed discussion of this aspect.)
...
\end{DoxyCodeInclude}
 (We refer to the subsection \doxysectlink{index_split}{Splitting corner elements in unstructured meshes to avoid locking}{2} in the section \doxysectlink{index_comm_ex}{Comments and Exercises}{1} for a discussion of the {\ttfamily split\+\_\+corner\+\_\+elements} flag).

Next, we set up the boundary lookup schemes that determine which elements are located next to which domain boundaries, and specify the IDs of the mesh boundaries that coincide with the in-\/ and outflow cross-\/sections. Note that this information reflects the specification of the boundary IDs in the {\ttfamily tetgen} {\ttfamily \texorpdfstring{$\ast$}{*}.poly} file.
...
\end{DoxyCodeInclude}
 The nodes on all other boundaries (i.\+e. the ones for which {\ttfamily done}\mbox{[}b\mbox{]} is still {\ttfamily false}) are subjected to no-\/slip conditions by pinning all three velocity components. (This approach facilitates the "{}extension"{} of the mesh discussed in section \doxysectlink{index_finer_mesh}{Creating a finer mesh and applying more appropriate outflow boundary conditions}{2}, but we note that, in general, keeping track of the boundary IDs associated with each physical boundary must be done "{}by hand"{}. )
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_traction}{}\doxysection{\texorpdfstring{Creating the fluid traction elements}{Creating the fluid traction elements}}\label{index_traction}
...
\begin{center} {\bfseries{How to spot "{}locking"{}\+:}} \end{center} 
\begin{DoxyItemize}
\item If a solution can be computed (using a direct solver) the occurrence of "{}locking"{} is easy to spot\+: Typically the pressure in over-\/constrained elements becomes extremely large while the rest of the flow field looks fairly normal. Iterative solvers may fail to converge. If you suspect that locking may have occurred, try re-\/computing the solution on a mesh with the {\ttfamily split\+\_\+corner\+\_\+elements} flag set to {\ttfamily true}.
\end{DoxyItemize}\DoxyHorRuler{0}
\hypertarget{index_finer_mesh}{}\doxysubsection{\texorpdfstring{Creating a finer mesh and applying more appropriate outflow boundary conditions}{Creating a finer mesh and applying more appropriate outflow boundary conditions}}\label{index_finer_mesh}
The flow field shown in the \doxysectlink{index_results}{Results}{1} section is clearly under-\/resolved and a much finer mesh would have to be used to fully resolve all flow features. The problem is particularly bad because we have imposed parallel outflow (i.\+e. flow in the $ z $-\/direction) at the ends of tubes whose axes are not aligned with the $ z $-\/axis. This creates thin outflow boundary layers within which the flow is forced to change direction as it exits the tubes. These observations motivate the following exercises that allow you to explore unstructured mesh generation.
