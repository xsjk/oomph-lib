...
\hypertarget{index}{}\label{index}\index{Interfaces, Free Surfaces and Surface Transport: Theory and implementation@{Interfaces, Free Surfaces and Surface Transport: Theory and implementation}}
...
\begin{DoxyItemize}
\item \doxysectlink{index_surface_representation}{Geometry of Surfaces}{2}
\item \doxysectlink{index_surface_gradient}{Differential Operators On A Surface}{2}
\item \doxysectlink{index_boundary_conditions}{Free Surface and Interface Boundary Conditions}{2}
\item \doxysectlink{index_surface_transport}{Surface Transport Equations}{2}
\end{DoxyItemize}
\item \doxysectlink{index_fs_implementation}{Implementation}{1}
\begin{DoxyItemize}
\item \doxysectlink{index_fluid_interface}{The Fluid\+Interface\+Element class}{2}
\item \doxysectlink{index_line}{The Line\+Derivatives class}{2}
\item \doxysectlink{index_axi}{The Axisymmetric\+Derivatives class}{2}
\item \doxysectlink{index_spine_formulation}{The Spine\+Line/\+Axi/\+Surface\+Fluid\+Interface\+Element classes}{1}
\item \doxysectlink{index_elastic_formulation}{The Elastic\+Line/\+Axi/\+Surface\+Fluid\+Interface\+Element classes}{1}
\end{DoxyItemize}
...
\DoxyHorRuler{0}
\hypertarget{index_surface_representation}{}\doxysubsection{\texorpdfstring{Geometry of Surfaces}{Geometry of Surfaces}}\label{index_surface_representation}
...
\DoxyHorRuler{0}
\hypertarget{index_surface_gradient}{}\doxysubsection{\texorpdfstring{Differential Operators On A Surface}{Differential Operators On A Surface}}\label{index_surface_gradient}
...
\DoxyHorRuler{0}
\hypertarget{index_boundary_conditions}{}\doxysubsection{\texorpdfstring{Free Surface and Interface Boundary Conditions}{Free Surface and Interface Boundary Conditions}}\label{index_boundary_conditions}
\hypertarget{index_dyn_con}{}\doxysubsubsection{\texorpdfstring{Dynamic condition}{Dynamic condition}}\label{index_dyn_con}
...
\sigma g^{\alpha\beta} \Big[\mbox{\boldmath$g$}_{\alpha}\Big]_{i}
 ...
 \zeta^{\beta} \quad (3) $. These are the terms that are implemented in {\ttfamily oomph-\/lib\textquotesingle{}s} free surface elements. Note that variations in surface tension are taken into account in this formulation without the explicit need to take its surface derivative. An important observation is that the intrinsic surface coordinates can be chosen to be the local coordinates of each element so that we do not need to introduce another set of coordinates.\hypertarget{index_kin_con}{}\doxysubsubsection{\texorpdfstring{Kinematic condition}{Kinematic condition}}\label{index_kin_con}
The kinematic condition is that "{}particles on the surface must  remain on the surface"{}. In other words the normal velocity of the surface must equal the normal rate of change of its position with time. The condition is compactly expressed in non-\/dimensional form as  \[ \left(\mbox{\boldmath$U$} - St \frac{\partial \mathbf{R}}{\partial
...
\DoxyHorRuler{0}
\hypertarget{index_surface_transport}{}\doxysubsection{\texorpdfstring{Surface Transport Equations}{Surface Transport Equations}}\label{index_surface_transport}
...
These are the equations implemented in {\ttfamily oomph-\/lib} using the definitions of surface derivatives given in \doxysectlink{index_surface_gradient}{Differential Operators On A Surface}{2}.

\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_fs_implementation}{}\doxysection{\texorpdfstring{Implementation}{Implementation}}\label{index_fs_implementation}
We will now discuss how the discrete versions of the equations derived above are actually implemented in {\ttfamily oomph-\/lib}. The basic idea is that the equations should be implemented independently of the specific element type and mesh-\/update strategy and a base class \doxylink{classoomph_1_1FluidInterfaceElement}{oomph\+::\+Fluid\+Interface\+Element} defines the generic functionality for all fluid interface elements. The only difference between the different surface geometries are in the definitions of the surface derivative operators and these are defined in specific classes \doxylink{classoomph_1_1LineDerivatives}{oomph\+::\+Line\+Derivatives} (1D surface), \doxylink{classoomph_1_1AxisymmetricDerivatives}{oomph\+::\+Axisymmetric\+Derivatives} and \doxylink{classoomph_1_1SurfaceDerivatives}{oomph\+::\+Surface\+Derivatives} (2D surface). The final specific element is created by using a special templated class that determines the node-\/update strategy and takes the base class, derivative class and bulk element as template arguments.

\DoxyHorRuler{0}
\hypertarget{index_fluid_interface}{}\doxysubsection{\texorpdfstring{The Fluid\+Interface\+Element class}{The FluidInterfaceElement class}}\label{index_fluid_interface}
The template-\/free \doxylink{classoomph_1_1FluidInterfaceElement}{oomph\+::\+Fluid\+Interface\+Element} class provides storage and member functions that are common to all free-\/surface and interface elements. The most important functions to be aware of are\+:
\begin{DoxyItemize}
...
\end{DoxyItemize}\hypertarget{index_line}{}\doxysubsection{\texorpdfstring{The Line\+Derivatives class}{The LineDerivatives class}}\label{index_line}
The class \doxylink{classoomph_1_1LineDerivatives}{oomph\+::\+Line\+Derivatives} implements the specific surface derivatives associated with a one-\/dimensional surface in a two-\/dimensional domain.\+The global coordinate system is Cartesian so its base vectors do not vary with the surface coordinates and  $ [\mbox{\boldmath$\psi$}_{,\beta}^{(F)}]_{i} =
\psi^{(F)}_{i,\beta}$; and the contribution to each component of the momentum equation is found by setting the appropriate component $ \psi^{(F)}_{i} = 0$ for $ i = 1,2 $.\hypertarget{index_axi}{}\doxysubsection{\texorpdfstring{The Axisymmetric\+Derivatives class}{The AxisymmetricDerivatives class}}\label{index_axi}
The class \doxylink{classoomph_1_1AxisymmetricDerivatives}{oomph\+::\+Axisymmetric\+Derivatives} implements the specific residuals associated with a two-\/dimensional surface in a three-\/dimensional domain, under the assumption of axisymmetry. Thus, the coordinate system is cylindrical polar $(r,z,\theta)$, but it is assumed that there are no variations in the $ \theta $ direction.
...
\psi_{r}^{(F)} = 0 $ and $ \psi_{z}^{(F)} = 0$. The difference from the {\ttfamily Line\+Derivatives} is the final term that accounts for the azimuthal curvature. In addition, we must also multiply all terms by the additional factor of $ r$ in the square-\/root of the determinant of the surface metric tensor.\hypertarget{index_surface}{}\doxysubsection{\texorpdfstring{The Surface\+Derivatives class}{The SurfaceDerivatives class}}\label{index_surface}
The class \doxylink{classoomph_1_1SurfaceDerivatives}{oomph\+::\+Surface\+Derivatives} implements the specific residuals associated with a general two-\/dimensional surface in a three-\/dimensional domain. Once again, the global coordinate is Cartesian, so the contribution to each momentum equation is found by setting the two other components of the test function to be zero.

\DoxyHorRuler{0}
\hypertarget{index_spine_formulation}{}\doxysection{\texorpdfstring{The Spine\+Line/\+Axi/\+Surface\+Fluid\+Interface\+Element classes}{The SpineLine/Axi/SurfaceFluidInterfaceElement classes}}\label{index_spine_formulation}
We shall discuss the "{}line"{} version of the elements, but the others are essentially the same.

The class \doxylink{classoomph_1_1SpineLineFluidInterfaceElement}{oomph\+::\+Spine\+Line\+Fluid\+Interface\+Element} is templated by the bulk element type, {\ttfamily ELEMENT}, and inherits from {\ttfamily Fluid\+Interface\+Element}, Line\+Derivatives and {\ttfamily Hijacked\texorpdfstring{$<$}{<}Spine\+Element\texorpdfstring{$<$}{<}Face\+Geometry$<$\+ELEMENT$>$} \texorpdfstring{$>$}{>} \texorpdfstring{$>$}{>}. The hijacking is only required for imposition of contact angle boundary conditions, see \href{../../static_single_layer/html/index.html}{\texttt{ another tutorial }} for more details.
...
\DoxyHorRuler{0}
\hypertarget{index_elastic_formulation}{}\doxysection{\texorpdfstring{The Elastic\+Line/\+Axi/\+Surface\+Fluid\+Interface\+Element classes}{The ElasticLine/Axi/SurfaceFluidInterfaceElement classes}}\label{index_elastic_formulation}
We shall discuss the "{}line"{} version of the elements, but the others are essentially the same.

The class \doxylink{classoomph_1_1ElasticLineFluidInterfaceElement}{oomph\+::\+Elastic\+Line\+Fluid\+Interface\+Element} is templated by the bulk element type, {\ttfamily ELEMENT}, and inherits from {\ttfamily Line\+Derivatives}, Fluid\+Interface\+Element and {\ttfamily Hijacked\texorpdfstring{$<$}{<}Face\+Geometry$<$\+ELEMENT$>$} \texorpdfstring{$>$}{>}. The hijacking is again only required for imposition of contact angle boundary conditions.
...
\end{DoxyCodeInclude}
 \hypertarget{index_surface}{}\doxysection{\texorpdfstring{The Surface\+Derivatives class}{The SurfaceDerivatives class}}\label{index_surface}
...
An important point is that the axisymmetric formulation of the surface divergence of a vector is not entirely trivial, as discussed in \doxysectlink{index_axi}{The Axisymmetric\+Derivatives class}{2}. In the context of surface transport the term that is always present is $
\mbox{\boldmath$\nabla$}_{\!\!_{S}}\mbox{\boldmath$\cdot$}
...
\mbox{\boldmath$U$}
 =  \frac{\left(\frac{\partial U_{r}}{\partial s} \frac{\partial
   ...
   z}{\partial s}\right)}{(r\textnormal{\textquotesingle})^{2} + (z\textnormal{\textquotesingle})^{2}} + \frac{U_{r}}{r},\] which is exactly the same form as the surface divergence of the vector test function derived in the section \doxysectlink{index_axi}{The Axisymmetric\+Derivatives class}{2}, as expected.
