\chapter{Demo problem\+: Large-\/amplitude non-\/axisymmetric oscillations of a thin-\/walled elastic ring}
\hypertarget{index}{}\label{index}\index{Demo problem: Large-\/amplitude non-\/axisymmetric oscillations of a thin-\/walled elastic ring@{Demo problem: Large-\/amplitude non-\/axisymmetric oscillations of a thin-\/walled elastic ring}}
...
The remaining lines in the plot show the ring\textquotesingle{}s kinetic and potential (i.\+e. the strain) energy and their sum. Up to $ t = T_{kick} $ , the external load does work on the ring and increases its kinetic and potential (strain) energy. Once the external load is "{}switched off"{} the total energy stored in the system should (and indeed does) remain constant. See the section \doxysectlink{index_energies}{The default non-\/dimensionalisation for the kinetic and potential (strain) energies}{2} for further details.
...
\end{DoxyCodeInclude}
 We also define the non-\/dimensional wall thickness $ h $ and the timescale ratio $ \Lambda^2 $. These are multiplied by powers of a scaling factor whose role will become apparent in the \doxysectlink{index_exercises}{Exercises}{2}. (By default, the scaling factor is set to 1.\+0 and does not play any role.)
...
\end{DoxyCodeInclude}
 Next, we build the problem with thirteen {\ttfamily Hermite\+Beam\+Elements} and a {\ttfamily Newmark$<$3$>$} timestepper ( \href{../../../linear_wave/two_d_linear_wave/html/index.html\#IC}{\texttt{ recall }} that a {\ttfamily Newmark$<$\+NSTEPS$>$} timestepper allocates and manages storage for the solution at {\ttfamily NSTEPS} previous timesteps; we shall illustrate this capability in the section \doxysectlink{index_prev}{How to retrieve the solution at previous timesteps}{2} ), before executing the timestepping loop.
...
The problem class is very similar to that used in the \href{../../steady_ring/html/index.html}{\texttt{ previous, steady example}} , but includes a few (obvious) additional functions that specify the initial conditions (\doxysectlink{index_IC}{Setting the initial condition}{1}) and perform the timestepping (\doxysectlink{index_unsteady_run}{The timestepping loop}{1}). We also provide two functions that allow us to dump the solution to disk (\doxysectlink{index_dump}{Writing a restart file}{1}) and to restart the time-\/dependent simulation (\doxysectlink{index_restart}{Restarting from a file}{1}).
...
\end{DoxyCodeInclude}
 \DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_dump}{}\doxysection{\texorpdfstring{Writing a restart file}{Writing a restart file}}\label{index_dump}
...
\end{DoxyCodeInclude}
 \DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_restart}{}\doxysection{\texorpdfstring{Restarting from a file}{Restarting from a file}}\label{index_restart}
...
\end{DoxyCodeInclude}
 \DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_IC}{}\doxysection{\texorpdfstring{Setting the initial condition}{Setting the initial condition}}\label{index_IC}
...
We start by converting the (optional) command line arguments into the flags that determine what mode the code is run in\+: Without command line arguments, we use the default assignments, as specified in \doxysectlink{index_constructor}{The problem constructor}{1}.
...
\hypertarget{index_timescale}{}\doxysubsection{\texorpdfstring{The default non-\/dimensionalisation of time}{The default non-dimensionalisation of time}}\label{index_timescale}
...
\DoxyHorRuler{0}
\hypertarget{index_energies}{}\doxysubsection{\texorpdfstring{The default non-\/dimensionalisation for the kinetic and potential (strain) energies}{The default non-dimensionalisation for the kinetic and potential (strain) energies}}\label{index_energies}
...
\DoxyHorRuler{0}
 \hypertarget{index_change_T}{}\doxysubsection{\texorpdfstring{Changing the timescale used to non-\/dimensionalise the equations}{Changing the timescale used to non-dimensionalise the equations}}\label{index_change_T}
...
\begin{DoxyEnumerate}
\item As discussed in \doxysectlink{index_timescale}{The default non-\/dimensionalisation of time}{2} , the period of the oscillation ( $ \approx 170 $ ) computed in the above example has a large numerical value because time was non-\/dimensionalised on a timescale that is representative for oscillations in which the ring\textquotesingle{}s motion is dominated by extensional (rather than bending) deformations. Furthermore, because the load is non-\/dimensionalised on the ring\textquotesingle{}s extensional (rather than its bending) stiffness, a very small load $ p_{cos} = 10^{-4} $ was sufficient to induce large-\/amplitude oscillations. ~\newline
 ~\newline
