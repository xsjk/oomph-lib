\chapter{Download/installation instructions}
\hypertarget{index}{}\label{index}\index{Download/installation instructions@{Download/installation instructions}}
\hypertarget{index_instructions}{}\doxysection{\texorpdfstring{The download/install process, step by step}{The download/install process, step by step}}\label{index_instructions}

\begin{DoxyEnumerate}
\item \doxysectlink{index_download}{Get the distribution}{1}
\item \doxysectlink{index_install}{Install the distribution}{1}
\begin{DoxyEnumerate}
\item \doxysectlink{index_standard_install}{Installation with {\ttfamily autogen.\+sh} }{2}
\item \doxysectlink{index_custom_install}{Configuration options}{2}
\end{DoxyEnumerate}
\item \doxysectlink{index_nav}{Finding your way through the distribution}{1}
\item \doxysectlink{index_external}{External (third-\/party) libraries}{1}
\begin{DoxyEnumerate}
\item \doxysectlink{index_external_src}{External (third-\/party) libraries that are distributed with oomph-\/lib}{2}
\begin{DoxyEnumerate}
\item \doxysectlink{index_blas}{Specifying an existing, local copy of the blas library}{3}
\item \doxysectlink{index_lapack}{Specifying an existing, local copy of the lapack library}{3}
\end{DoxyEnumerate}
\item \doxysectlink{index_external_dist_cgal_etc}{External (third-\/party) libraries whose tar files are distributed with oomph-\/lib}{2}
\item \doxysectlink{index_external_dist}{External (third-\/party) libraries that are not distributed with oomph-\/lib}{2}
\begin{DoxyEnumerate}
\item \doxysectlink{index_hypre}{Hypre}{3}
\item \doxysectlink{index_trilinos}{Trilinos}{3}
\item \doxysectlink{index_mumps}{MUMPS and Sca\+LAPACK}{3}
\end{DoxyEnumerate}
\end{DoxyEnumerate}
\item \doxysectlink{index_own}{How to write your own code and link it against oomph-\/lib\textquotesingle{}s library/libraries}{1}
\begin{DoxyEnumerate}
\item \doxysectlink{index_have_auto_tools}{Writing/linking your own driver codes under autotools control}{2}
\begin{DoxyEnumerate}
\item \doxysectlink{index_add_driver}{Adding your own driver codes}{3}
\item \doxysectlink{index_add_src}{Adding new libraries and linking against them from driver codes}{3}
\end{DoxyEnumerate}
\item \doxysectlink{index_dont_have_auto_tools}{Writing/linking user code without autotools\+: How do I treat {\ttfamily oomph-\/lib} simply as a library?}{2}
\end{DoxyEnumerate}
\end{DoxyEnumerate}\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_download}{}\doxysection{\texorpdfstring{Get the distribution}{Get the distribution}}\label{index_download}
...
\hypertarget{index_install}{}\doxysection{\texorpdfstring{Install the distribution}{Install the distribution}}\label{index_install}
\hypertarget{index_standard_install}{}\doxysubsection{\texorpdfstring{Installation with \textbackslash{}c autogen.\+sh}{Installation with \textbackslash{}c autogen.sh}}\label{index_standard_install}
...
By default, {\ttfamily autogen}.{\ttfamily sh} will build the library and the demo codes with certain default settings (using the gcc compilers with full optimisation, no debugging, no PARANOIA, and fully installed header files rather than symbolic links). These are appropriate if you wish to install the library once-\/and-\/for-\/all and do not anticipate any changes to their sources. The section \doxysectlink{index_custom_install}{Configuration options}{2} provides more details on the various options). {\ttfamily autogen.\+sh} will display the configure options and ask for confirmation that they are appropriate. If you are unsure if they are, simply hit return -- the default will be fine.
...
\end{DoxyVerb}
 in {\ttfamily oomph-\/lib}\textquotesingle{}s top level directory. Here are some options that we use frequently\+:\hypertarget{index_no_doc}{}\doxysubsubsection{\texorpdfstring{Suppressing the build of the documentation\+:}{Suppressing the build of the documentation:}}\label{index_no_doc}
...
\end{DoxyVerb}
 \hypertarget{index_no_pdf}{}\doxysubsubsection{\texorpdfstring{Suppressing the build of the pdf version of the documentation\+:}{Suppressing the build of the pdf version of the documentation:}}\label{index_no_pdf}
...
\end{DoxyCode}
 is common.\hypertarget{index_bounds}{}\doxysubsubsection{\texorpdfstring{Range checking}{Range checking}}\label{index_bounds}
...
\DoxyHorRuler{0}
\hypertarget{index_nav}{}\doxysection{\texorpdfstring{Finding your way through the distribution}{Finding your way through the distribution}}\label{index_nav}
The {\ttfamily oomph-\/lib} distribution has several main sub-\/directories\+:\hypertarget{index_src}{}\doxysubsection{\texorpdfstring{The src directory}{The src directory}}\label{index_src}
...
\end{DoxyItemize}Note that the installation of these libraries is not quick. We therefore provide the option to (i) suppress their installation (in which case {\ttfamily oomph-\/lib} will employ a sub-\/optimal "{}locate\+\_\+zeta"{} algorithm in its multi-\/domain algorithms) or to (ii) link against already existing installations of the libraries; see \doxysectlink{index_external_dist_cgal_etc}{External (third-\/party) libraries whose tar files are distributed with oomph-\/lib}{2} for details.\hypertarget{index_trilinos_hypre_external}{}\doxysubsubsection{\texorpdfstring{External distributions that are not built by default}{External distributions that are not built by default}}\label{index_trilinos_hypre_external}
{\ttfamily oomph-\/lib} provides interfaces to various optional third-\/party libraries whose sources we deemed to be too big to be included in the {\ttfamily oomph-\/lib} distribution. If you wish to use these we expect you to install them yourself. To facilitate this task, we provide the option to let the {\ttfamily oomph-\/lib} build machinery perform the installation for you. If you place a copy of the tar file into the appropriate sub-\/directory in {\ttfamily external\+\_\+distributions}, {\ttfamily oomph-\/lib} will build and install the library for you; see \doxysectlink{index_external_dist}{External (third-\/party) libraries that are not distributed with oomph-\/lib}{2} for details. (Note that {\ttfamily oomph-\/lib} is fully functional without these libraries -- if the libraries are not available the build process ignores any {\ttfamily oomph-\/lib} code that depends on them.)\hypertarget{index_demo_drivers}{}\doxysubsection{\texorpdfstring{The demo\+\_\+drivers directory}{The demo\_drivers directory}}\label{index_demo_drivers}
...
\end{DoxyItemize}By default {\ttfamily oomph-\/lib} automatically builds and links against these.\hypertarget{index_blas}{}\doxysubsubsection{\texorpdfstring{Specifying an existing, local copy of the blas library}{Specifying an existing, local copy of the blas library}}\label{index_blas}
...
\end{DoxyCode}
\hypertarget{index_lapack}{}\doxysubsubsection{\texorpdfstring{Specifying an existing, local copy of the lapack library}{Specifying an existing, local copy of the lapack library}}\label{index_lapack}
...
\end{DoxyCode}
\hypertarget{index_external_dist_cgal_etc}{}\doxysubsection{\texorpdfstring{External (third-\/party) libraries whose tar files are distributed with oomph-\/lib}{External (third-party) libraries whose tar files are distributed with oomph-lib}}\label{index_external_dist_cgal_etc}
By default {\ttfamily oomph-\/lib} builds CGAL, the Computational Geometry Algorithms Library, \href{http://www.cgal.org}{\texttt{ http\+://www.\+cgal.\+org}}. This library requires three other libraries which we also install\+:
\begin{DoxyItemize}
\item The GNU Multiple Precision Arithmetic Library (GMP), \href{https://gmplib.org}{\texttt{ https\+://gmplib.\+org}}.
\item The GNU MPFR Library \href{http://www.mpfr.org}{\texttt{ https\+://gmplib.\+org}}.
\item The Boost library, \href{http://www.boost.org}{\texttt{ http\+://www.\+boost.\+org}}.
\end{DoxyItemize}These four libraries are built from tar files that we downloaded from the relevant webpages and then included into the {\ttfamily oomph-\/lib} distribution. We adopted this procedure to ensure that the versions of the libraries are consistent with each other.\hypertarget{index_cgal_default_install}{}\doxysubsubsection{\texorpdfstring{Default CGAL installation.}{Default CGAL installation.}}\label{index_cgal_default_install}
By default the four libraries are installed within {\ttfamily oomph-\/lib\textquotesingle{}s} {\ttfamily external\+\_\+distributions} directory, and the paths to the relevant {\ttfamily lib} and {\ttfamily include} directories are propagated to {\ttfamily oomph-\/lib\textquotesingle{}s} {\ttfamily Makefile} s. The installations are deleted by "{}make clean"{} or "{}make distclean"{}, i.\+e. they are treated like any other {\ttfamily oomph-\/lib} code.\hypertarget{index_cgal_permanent_install}{}\doxysubsubsection{\texorpdfstring{Installing CGAL in a permanent location}{Installing CGAL in a permanent location}}\label{index_cgal_permanent_install}
...
hierher auto?\hypertarget{index_cgal_no_install}{}\doxysubsubsection{\texorpdfstring{Suppressing the CGAL installation}{Suppressing the CGAL installation}}\label{index_cgal_no_install}
...
\end{DoxyCode}
 In this case {\ttfamily oomph-\/lib} will employ a sub-\/optimal "{}locate\+\_\+zeta"{} algorithm in its multi-\/domain algorithms.\hypertarget{index_external_dist}{}\doxysubsection{\texorpdfstring{External (third-\/party) libraries that are not distributed with oomph-\/lib}{External (third-party) libraries that are not distributed with oomph-lib}}\label{index_external_dist}
...
or download them one-\/by-\/one using the links provided below.\hypertarget{index_hypre}{}\doxysubsubsection{\texorpdfstring{Hypre}{Hypre}}\label{index_hypre}
...
\DoxyHorRuler{0}
\hypertarget{index_mumps}{}\doxysubsubsection{\texorpdfstring{MUMPS and Sca\+LAPACK}{MUMPS and ScaLAPACK}}\label{index_mumps}
...
\DoxyHorRuler{0}
\hypertarget{index_own}{}\doxysection{\texorpdfstring{How to write your own code and link it against oomph-\/lib\textquotesingle{}s library/libraries}{How to write your own code and link it against oomph-lib's library/libraries}}\label{index_own}
If you followed the instructions so far, you will be able to install {\ttfamily oomph-\/lib} and run the demo codes that are provided in the {\ttfamily demo\+\_\+drivers} directory. Great! Now on to the next step\+: How do you write your own codes and link them against {\ttfamily oomph-\/lib}? There are two options, depending on whether you have (or are willing to install) the gnu autotools \href{http://www.gnu.org/software/autoconf/}{\texttt{ autoconf}}, \href{http://www.gnu.org/software/automake/}{\texttt{ automake}} and \href{http://www.gnu.org/software/libtool/}{\texttt{ libtool}} on your machine.\hypertarget{index_have_auto_tools}{}\doxysubsection{\texorpdfstring{Writing/linking your own driver codes under autotools control}{Writing/linking your own driver codes under autotools control}}\label{index_have_auto_tools}
\hypertarget{index_add_driver}{}\doxysubsubsection{\texorpdfstring{Adding your own driver codes}{Adding your own driver codes}}\label{index_add_driver}
...
\begin{DoxyItemize}
\item Lines that start with a "{}\#"{} are comments. ~\newline
~\newline

\item The first (non-\/comment) line in the above file specifies the name(s) of the executable(s) that will be created by make. These names must be the same as those in the {\ttfamily \texorpdfstring{$\ast$}{*}\+\_\+\+SOURCES} and {\ttfamily \texorpdfstring{$\ast$}{*}\+\_\+\+LDADD} variables. ~\newline
~\newline
...
\item If you have multiple driver codes, add the name of all executables to the {\ttfamily noinst\+\_\+\+PROGRAMS} variable, and specify the {\ttfamily \texorpdfstring{$\ast$}{*}\+\_\+\+SOURCES} and {\ttfamily \texorpdfstring{$\ast$}{*}\+\_\+\+LDADD} variables for each one. ~\newline
~\newline
...
\end{DoxyEnumerate}\hypertarget{index_add_src}{}\doxysubsubsection{\texorpdfstring{Adding new libraries and linking against them from driver codes}{Adding new libraries and linking against them from driver codes}}\label{index_add_src}
