\chapter{(Not-\/\+So-\/)Quick Users Guide}
\hypertarget{index}{}\label{index}\index{(Not-\/So-\/)Quick Users Guide@{(Not-\/So-\/)Quick Users Guide}}
The purpose of this document is to provide a "{}quick"{} introduction to the fundamental objects in {\ttfamily oomph-\/lib}. Assuming that you
\begin{DoxyItemize}
...
\begin{DoxyItemize}
\item If {\ttfamily oomph-\/lib\textquotesingle{}s} existing {\ttfamily Mesh} objects and element types are sufficient to discretise the problem, the generation of a fully-\/functional {\ttfamily Problem} object is straightforward. We shall discuss this case in the section \doxysectlink{index_problem}{How to build a Problem}{1}.
\item If {\ttfamily oomph-\/lib} provides the required element type but none of the available meshes are suitable, you will have to write your own {\ttfamily Mesh} object. We shall discuss this in the section \doxysectlink{index_mesh}{How to build a Mesh}{1}. It is also possible to create {\ttfamily oomph-\/lib} meshes based on the output from third-\/party (commercial) mesh generators. This is illustrated in \href{../../meshes/third_party_meshes/html/index.html}{\texttt{ another example.}}
...
\begin{DoxyItemize}
\item In the section \doxysectlink{index_FEequations}{How to implement a new system of equations as a specific Finite\+Element}{2} we describe how to discretise a new system of equations, using existing geometric elements.
\item In the section \doxysectlink{index_FEgeom}{How to build a new geometric element}{2} we discuss how to formulate new geometric elements.
\end{DoxyItemize}
...
by isoparametric two-\/node elements. (Please consult the \href{../../intro/html/index.html}{\texttt{ Introduction}} for a review of the relevant finite element theory.) The source code for this example (\href{../../../demo_drivers/poisson/one_d_poisson_generic_only/one_d_poisson_generic_only.cc}{\texttt{ one\+\_\+d\+\_\+poisson\+\_\+generic\+\_\+only.\+cc}}) is self-\/contained and uses only objects from the "{}generic"{} part of the {\ttfamily oomph-\/lib} library. \mbox{[}{\ttfamily oomph-\/lib} does, of course, provide 1D meshes and Poisson elements but we deliberately ignore these because we wish to illustrate how to build such objects from scratch. The alternative example code \href{../../../demo_drivers/poisson/one_d_poisson/one_d_poisson.cc}{\texttt{ one\+\_\+d\+\_\+poisson.\+cc}}, discussed in \href{../../poisson/one_d_poisson/html/index.html}{\texttt{ another example}} illustrates the implementation of the same problem using existing library objects.\mbox{]}

The main purpose of this document is to provide a {\itshape "{}quick"{}} introduction to {\ttfamily oomph-\/lib\textquotesingle{}s} fundamental objects, so the example classes presented below are fully-\/functional but very basic. We provide exercises at the end of each section to allow the reader to explore various straightforward improvements, most of which are implemented in the corresponding objects in the library. Finally, the section \doxysectlink{index_comments}{Further comments}{1} provides a discussion of some general design principles that should be respected when creating new classes for use with the library.

\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_problem}{}\doxysection{\texorpdfstring{How to build a Problem}{How to build a Problem}}\label{index_problem}
...
\end{DoxyItemize}\hypertarget{index_spec}{}\doxysubsection{\texorpdfstring{The constructor\+: Some\+Specific\+Problem(...)}{The constructor: SomeSpecificProblem(...)}}\label{index_spec}
...
\end{DoxyItemize}\hypertarget{index_actbef}{}\doxysubsection{\texorpdfstring{The function Problem\+::actions\+\_\+before\+\_\+newton\+\_\+solve()}{The function Problem::actions\_before\_newton\_solve()}}\label{index_actbef}
...
\DoxyHorRuler{0}
\hypertarget{index_full}{}\doxysubsection{\texorpdfstring{An example of a "{}complete"{} specific Problem}{An example of a "complete" specific Problem}}\label{index_full}
...
\DoxyHorRuler{0}
 \hypertarget{index_problem_exercise}{}\doxysubsection{\texorpdfstring{Exercises\+:}{Exercises:}}\label{index_problem_exercise}
...
\item Compile and run the example code \href{../../../demo_drivers/poisson/one_d_poisson_generic_only/one_d_poisson_generic_only.cc}{\texttt{ one\+\_\+d\+\_\+poisson\+\_\+generic\+\_\+only.\+cc}} and compare the numerical results against the (fish-\/shaped) exact solution  \[
  ...
  \] (The sign of the source function is an optional argument to the problem constructor. If no argument is specified, the + sign is used.) Modify the {\ttfamily actions\+\_\+after\+\_\+newton\+\_\+solve()} function so that it writes the exact solution into another file, "{}exact\+\_\+solution.\+dat"{}, say.
...
\end{DoxyItemize}\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_mesh}{}\doxysection{\texorpdfstring{How to build a Mesh}{How to build a Mesh}}\label{index_mesh}
{\ttfamily oomph-\/lib} provides a large number of fully-\/functional {\ttfamily Mesh} objects. Many of these meshes can easily be adapted to discretise other domains, provided the domain has the same topology as the original {\ttfamily Mesh}. We shall illustrate this in section \doxysectlink{index_distorted_mesh}{How to adapt an existing Mesh to a different domain shape}{2} below.\hypertarget{index_new_mesh}{}\doxysubsection{\texorpdfstring{How to build a completely new Mesh}{How to build a completely new Mesh}}\label{index_new_mesh}
...
This allows the {\ttfamily Specific\+Mesh} to be used with any {\ttfamily Finite\+Element} that is based on the same geometric element type (see the section \doxysectlink{index_FEgeom}{How to build a new geometric element}{2} for a more detailed discussion of geometric elements).
...
\end{DoxyCode}
\hypertarget{index_distorted_mesh}{}\doxysubsection{\texorpdfstring{How to adapt an existing Mesh to a different domain shape}{How to adapt an existing Mesh to a different domain shape}}\label{index_distorted_mesh}
Given that mesh generation tends to be a fairly tedious process, one should always check whether it is possible to deform an existing {\ttfamily Mesh} into a shape that matches the required domain. Provided that the new domain has the same topology as the domain represented by the original {\ttfamily Mesh}, this can always be done by re-\/positioning the Nodes. Such "{}deformed"{} {\ttfamily Meshes} should be implemented via inheritance, by deriving the new {\ttfamily Mesh} from an existing one.
...
\DoxyHorRuler{0}
\hypertarget{index_mes_exercise}{}\doxysubsection{\texorpdfstring{Exercise\+:}{Exercise:}}\label{index_mes_exercise}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_finiteelement}{}\doxysection{\texorpdfstring{How to build a Finite\+Element}{How to build a FiniteElement}}\label{index_finiteelement}
...
\begin{DoxyItemize}
\item \doxysectlink{index_FEequations}{How to implement a new system of equations as a specific Finite\+Element}{2}
\item \doxysectlink{index_FEgeom}{How to build a new geometric element}{2}
\end{DoxyItemize}

\DoxyHorRuler{0}
\hypertarget{index_FEequations}{}\doxysubsection{\texorpdfstring{How to implement a new system of equations as a specific Finite\+Element}{How to implement a new system of equations as a specific FiniteElement}}\label{index_FEequations}
...
\end{DoxyCodeInclude}
 We will now discuss the implementation of the generic {\ttfamily Finite\+Element} member functions for the specific {\ttfamily Two\+Node\+Poisson\+Element\+:} \hypertarget{index_Fereq_nvalue}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::required\+\_\+nvalue(n)}{The function FiniteElement::required\_nvalue(n)}}\label{index_Fereq_nvalue}
...
\end{DoxyCodeInclude}
 The function is used by {\ttfamily Finite\+Element\+::construct\+\_\+node(...)} to determine the amount of storage to be allocated at each of the element\textquotesingle{}s {\ttfamily Nodes}.\hypertarget{index_residuals}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::get\+\_\+residuals(residuals)}{The function FiniteElement::get\_residuals(residuals)}}\label{index_residuals}
...
\end{DoxyCodeInclude}
 \hypertarget{index_jac}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::get\+\_\+jacobian(residuals,jacobian)}{The function FiniteElement::get\_jacobian(residuals,jacobian)}}\label{index_jac}
...
\end{DoxyCode}
\hypertarget{index_FEout}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::output(out\+\_\+stream)}{The function FiniteElement::output(out\_stream)}}\label{index_FEout}
...
\end{DoxyCodeInclude}
 \DoxyHorRuler{0}
 \hypertarget{index_FE_exercise}{}\doxysubsection{\texorpdfstring{Exercises\+:}{Exercises:}}\label{index_FE_exercise}
...
\end{DoxyItemize}The pointer to the source function (typically defined in a suitable namespace in the "{}user\textquotesingle{}s"{} driver code) can now be set in the Problem constructor.
...
\DoxyHorRuler{0}
\hypertarget{index_FEgeom}{}\doxysubsection{\texorpdfstring{How to build a new geometric element}{How to build a new geometric element}}\label{index_FEgeom}
...
\item {\bfseries{Optional\+:}} If the spatial dimension (= the number of Eulerian coordinates) required at all Nodes in the element is different from the spatial dimension of the element then set the "{}nodal dimension"{} by calling the protected member function 
\begin{DoxyCode}{0}
...
\item {\bfseries{Optional\+:}} If the Eulerian coordinate (position) is interpolated using generalised coordinates, e.\+g. vector-\/valued shape functions, or Hermite-\/type interpolation, set the number of generalised coordinates or "{}position types"{} by using the protected member function 
\begin{DoxyCode}{0}
...
\item The function that specifies the number of Nodes along the element\textquotesingle{}s 1D "{}edges"{} ~\newline
...
\end{DoxyCodeInclude}
 Here is the implementation of the generic {\ttfamily Finite\+Element} member functions for our specific geometric {\ttfamily Finite\+Element\+:} \hypertarget{index_Geomconst}{}\doxysubsubsection{\texorpdfstring{The constructor\+: Two\+Node\+Geometric\+Element()}{The constructor: TwoNodeGeometricElement()}}\label{index_Geomconst}
...
\end{DoxyCodeInclude}
 \hypertarget{index_shape}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::shape(s,psi)}{The function FiniteElement::shape(s,psi)}}\label{index_shape}
...
\end{DoxyCodeInclude}
 \hypertarget{index_dshape}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::dshape\+\_\+local(s,psi,dpsids)}{The function FiniteElement::dshape\_local(s,psi,dpsids)}}\label{index_dshape}
...
\end{DoxyCodeInclude}
\hypertarget{index_nnode_1d}{}\doxysubsubsection{\texorpdfstring{The function Finite\+Element\+::nnode\+\_\+1d()}{The function FiniteElement::nnode\_1d()}}\label{index_nnode_1d}
Return the number of nodes along the element\textquotesingle{}s 1d "{}edge"{} -- for a one-\/dimensional element, this is obviously the same as the number of nodes\+: 
\begin{DoxyCodeInclude}{0}
\DoxyCodeLine{\ \textcolor{keywordtype}{unsigned}\ nnode\_1d()\textcolor{keyword}{\ const\ }\{\textcolor{keywordflow}{return}\ 2;\}}

\end{DoxyCodeInclude}
 \DoxyHorRuler{0}
 \hypertarget{index_geom_element_exercise}{}\doxysubsection{\texorpdfstring{Exercises\+:}{Exercises:}}\label{index_geom_element_exercise}
...
\begin{DoxyItemize}
\item \doxysectlink{index_dim_independent}{Dimension-\/independent implementation}{2}
\item \doxysectlink{index_pre_compute_psi}{Using pre-\/computed shape functions and the Storable\+Shape\+Element$<$\+ELEMENT$>$ class}{2}
\item \doxysectlink{index_equation_classes}{Further sub-\/division of specific element classes -- equation classes.}{2}
\item \doxysectlink{index_paranoia}{Implement extensive (but optional) self tests in all classes}{2}
\item \doxysectlink{index_magic}{The use of "{}magic numbers"{}}{2}
\item \doxysectlink{index_multi_physics}{Facilitating the use of specific elements in multi-\/physics problems}{2}
\end{DoxyItemize}

\DoxyHorRuler{0}
\hypertarget{index_dim_independent}{}\doxysubsection{\texorpdfstring{Dimension-\/independent implementation}{Dimension-independent implementation}}\label{index_dim_independent}
The exercises in the section \doxysectlink{index_FEgeom}{How to build a new geometric element}{2} suggest the use of templating to implement families of objects. This idea also permits the implementation of (geometric and specific) {\ttfamily Finite\+Elements} in arbitrary spatial dimensions. For instance, {\ttfamily oomph-\/lib\textquotesingle{}s} {\ttfamily QElement} class represents the family of (line/quadrilateral/brick-\/shaped) geometric elements with an arbitrary number of nodes along the elements\textquotesingle{} 1D edges. Thus, a {\ttfamily QElement$<$1,3$>$} is a three-\/node 1D line element, a {\ttfamily QElement$<$3,2$>$} is an eight-\/node 3D brick element, etc.
...
\end{DoxyCode}
 Once these functions have been called, any subsequent calls to the {\ttfamily \texorpdfstring{$\ast$}{*}\+\_\+eulerian\+\_\+at\+\_\+knot}(...) functions return the stored values. To revert to the case in which the derivatives are re-\/computed "{}on the fly"{}, the storage for the derivatives must be deleted by ~\newline
 calling 
\begin{DoxyCode}{0}
...
\DoxyHorRuler{0}
\hypertarget{index_equation_classes}{}\doxysubsection{\texorpdfstring{Further sub-\/division of specific element classes – equation classes.}{Further sub-division of specific element classes – equation classes.}}\label{index_equation_classes}
We suggested that any newly developed elements should be constructed in a three-\/level hierarchy, {\ttfamily Finite\+Element} -- Geometric Element -- Specific {\ttfamily Finite\+Element}. Most finite elements in {\ttfamily oomph-\/lib} incorporate an additional intermediate "{}equation class"{} which implements the computation of the element residual vector and the element Jacobian matrix in terms of abstract shape and test functions, defined as pure virtual functions in the "{}equation class"{}. This makes it easy to change the specific element formulation, without having to re-\/implement the weak form of the governing equation.
...
At large Peclet number, the Galerkin discretisation of the advection diffusion equation is well-\/known to produce spurious "{}wiggles"{} in the solution. These can be suppressed by SUPG stabilisation which employs test functions, $ \psi_j^{(test)}
...
\DoxyHorRuler{0}
\hypertarget{index_magic}{}\doxysubsection{\texorpdfstring{The use of "{}magic numbers"{}}{The use of "magic numbers"}}\label{index_magic}
In general, we set the equation number to -\/1 to indicate that a nodal value is pinned, i.\+e. prescribed by boundary conditions. This convention is used throughout {\ttfamily oomph-\/lib}, but the direct use of -\/1 as a (bad!) "{}magic number"{} is generally avoided. Instead we refer to the static data member of the {\ttfamily Data} class 
\begin{DoxyCode}{0}
...
\DoxyHorRuler{0}
\hypertarget{index_multi_physics}{}\doxysubsection{\texorpdfstring{Facilitating the use of specific elements in multi-\/physics problems}{Facilitating the use of specific elements in multi-physics problems}}\label{index_multi_physics}
As the name of the library suggests, {\ttfamily oomph-\/lib\textquotesingle{}s} existing "{}single-\/physics"{} elements are (and any newly designed ones should be) designed so that they can easily be used in multi-\/physics problems. We anticipate two types of multi-\/physics interactions\+:
\begin{DoxyItemize}
...
\begin{DoxyItemize}
\item \doxysectlink{index_dont_wipe}{Rule 1\+: Elements must only initialise their own entries in the element residual vector and the element Jacobian matrix}{3}
\item \doxysectlink{index_virtual}{Rule 2\+: Forcing functions, etc. should be implemented as uniquely-\/named virtual functions}{3}
\item \doxysectlink{index_index_fcts}{Rule 3\+: Nodal values should only be accessed indirectly via index functions that can be overloaded in derived multi-\/physics elements}{3}
\item \doxysectlink{index_mesh_movement}{Rule 4\+: Time derivatives should be implemented in the ALE formulation to allow for moving meshes.}{3}
\end{DoxyItemize}

\DoxyHorRuler{0}
\hypertarget{index_dont_wipe}{}\doxysubsubsection{\texorpdfstring{Rule 1\+: Elements must only initialise their own entries in the element residual vector and the element Jacobian matrix}{Rule 1: Elements must only initialise their own entries in the element residual vector and the element Jacobian matrix}}\label{index_dont_wipe}
The computation of the element residual vector and the element Jacobian matrix typically involves a loop over the element\textquotesingle{}s integration points, in which each integration point adds a further contribution to the entries. In multi-\/physics problems it is crucial that the initialisation of the residual vector and the Jacobian matrix only sets the element\textquotesingle{}s "{}own"{} entries to zero.

To illustrate this point, consider what would happen if we used the {\ttfamily Two\+Node\+Poisson\+Element\+::get\+\_\+jacobian(...)} function, discussed in the section \doxysectlink{index_jac}{The function Finite\+Element\+::get\+\_\+jacobian(residuals,jacobian)}{3}, in a derived multi-\/physics element, which combines the {\ttfamily Two\+Node\+Poisson\+Element} with another element, {\ttfamily Two\+Node\+Some\+Other\+Equation\+Element}, say. Assume that we implement the function {\ttfamily get\+\_\+jacobian(...)} of the combined element so that it first calls the function {\ttfamily Two\+Node\+Some\+Other\+Equation\+Element\+::get\+\_\+jacobian(...)} to determine the first diagonal block in the combined Jacobian matrix. When we call {\ttfamily Two\+Node\+Poisson\+Element\+::get\+\_\+jacobian(...)} to compute the entries in the second diagonal block, the initialisation loop
...
It is, therefore, recommended that authors of "{}single-\/physics"{} elements, overload {\ttfamily fill\+\_\+in\+\_\+contribution\+\_\+to\+\_\+residuals(...)} and {\ttfamily fill\+\_\+in\+\_\+contribution\+\_\+to\+\_\+jacobian(...)}, rather than {\ttfamily get\+\_\+residuals()} and {\ttfamily get\+\_\+jacobian(...)}, respectively. A further advantage of the implementation is that the author need not worry about initialisation of the residuals vector or the Jacobian matrix when using the {\ttfamily "{}fill\+\_\+in\+\_\+"{}} rather than the {\ttfamily "{}get\+\_\+"{}} functions.\hypertarget{index_virtual}{}\doxysubsubsection{\texorpdfstring{Rule 2\+: Forcing functions, etc. should be implemented as  uniquely-\/named virtual functions}{Rule 2: Forcing functions, etc. should be implemented as  uniquely-named virtual functions}}\label{index_virtual}
