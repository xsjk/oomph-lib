...
\hypertarget{index}{}\label{index}\index{Demo problem: Compressible and incompressible behaviour@{Demo problem: Compressible and incompressible behaviour}}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_exact}{}\doxysection{\texorpdfstring{An exact solution}{An exact solution}}\label{index_exact}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_wrapper}{}\doxysection{\texorpdfstring{Modifying the element\textquotesingle{}s output function}{Modifying the element's output function}}\label{index_wrapper}
...
\end{DoxyCodeInclude}
 Finally, we pick a control node to document the solid\textquotesingle{}s load-\/displacement characteristics and apply the boundary conditions (no displacements normal to the "{}slippery"{} walls) before setting up the equation numbering scheme.
