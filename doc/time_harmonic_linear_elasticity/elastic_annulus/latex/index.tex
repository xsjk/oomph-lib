\chapter{The equations of time-\/harmonic linear elasticity}
\hypertarget{index}{}\label{index}\index{The equations of time-\/harmonic linear elasticity@{The equations of time-\/harmonic linear elasticity}}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_test}{}\doxysection{\texorpdfstring{A test problem\+: Oscillations of an elastic annulus}{A test problem: Oscillations of an elastic annulus}}\label{index_test}
...
The {\ttfamily Problem} class is very simple. As in other problems with Neumann boundary conditions, we provide separate meshes for the "{}bulk"{} elements and the face elements that apply the traction boundary conditions. The latter are attached to the relevant faces of the bulk elements by the function {\ttfamily create\+\_\+traction\+\_\+elements()}.
...
We begin by building the "{}bulk"{} solid mesh, specifying the presence of the gap (and its width) if necessary. If there is no gap, the mesh is periodic; see \doxysectlink{index_comments}{Comments}{2} for a more detailed discussion of how the mesh for this problem is constructed.
...
\end{DoxyCodeInclude}
 We apply the displacement boundary conditions by pinning the real and imaginary part of the two displacement components at all nodes on boundary 0. We then impose a purely radial displacement on the real and imaginary parts of the displacement field, using the function defined in the {\ttfamily \doxylink{namespaceGlobal__Parameters}{Global\+\_\+\+Parameters}} namespace\+:
...
\end{DoxyItemize}and
\begin{DoxyItemize}
\item for the problem with the "{}gap"{} in the annular region.
\end{DoxyItemize}~\newline
~\newline
