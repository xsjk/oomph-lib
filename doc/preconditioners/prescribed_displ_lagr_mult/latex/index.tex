...
\hypertarget{index}{}\label{index}\index{Demo problem: A preconditioner for the solution of (pseudo-\/)solid mechanics problems with prescribed boundary motions@{Demo problem: A preconditioner for the solution of (pseudo-\/)solid mechanics problems with prescribed boundary motions}}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_model}{}\doxysection{\texorpdfstring{Theory\+: Problem formulation, the resulting linear system and the preconditioner}{Theory: Problem formulation, the resulting linear system and the preconditioner}}\label{index_model}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_implementation}{}\doxysection{\texorpdfstring{Implementation and use of the preconditioner}{Implementation and use of the preconditioner}}\label{index_implementation}
...
\begin{DoxyItemize}
\item {\bfseries{"{}\+Super\+LU"{}}} corresponds to the exact implementation of the preconditioner $ E_{\rm PS} $ in which the linear systems involving the matrix $ E_{\rm PS} $ and the two squared mass matrices, $ M_{\rm x}^2 $ and $ M_{\rm y}^2 $, are solved by Super\+LU.
\item {\bfseries{"{}\+Upper triangular E"{}}} corresponds to the case in which $ E_{\rm PS} $ is replaced by its upper block-\/triangular approximation (corresponding to $ \tilde{\cal P}_{\rm PS} $) but the four linear solves (two involving the diagonal blocks of $ E_{\rm PS} $; two involving the squared mass matrices, $ M_{\rm x}^2 $ and $ M_{\rm y}^2 $), are performed by Super\+LU.
\item {\bfseries{"{}\+Upper triangular E; Hypre/\+CG"{}}} are the results obtained when using the same block approximation of the preconditioner, but using two AMG cycles (performed with Hypre) for the approximate solution of the diagonal blocks of $ E_{\rm PS} $, and diagonally preconditioned CG as the approximate solver for the mass matrices.
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_modifications}{}\doxysection{\texorpdfstring{Modifications to the driver code}{Modifications to the driver code}}\label{index_modifications}
...
\DoxyHorRuler{0}
\hypertarget{index_reclass}{}\doxysubsection{\texorpdfstring{(\+Re-\/)classification of the dof types}{(Re-)classification of the dof types}}\label{index_reclass}
...
\DoxyHorRuler{0}
\hypertarget{index_driver}{}\doxysubsection{\texorpdfstring{Modifications to the main code}{Modifications to the main code}}\label{index_driver}
...
\end{DoxyCodeInclude}
 We then assign the command line flags that were specified and document the ones that were recognised before building the problem, specifying the "{}wrapped"{} version of the {\ttfamily Refineable\+QPVDElement} as the template parameter.
...
\DoxyHorRuler{0}
\hypertarget{index_constr}{}\doxysubsection{\texorpdfstring{Modifications to the problem constructor}{Modifications to the problem constructor}}\label{index_constr}
...
\end{DoxyItemize}\hypertarget{index_ex}{}\doxysubsection{\texorpdfstring{Exercises}{Exercises}}\label{index_ex}
