\chapter{Example problem\+: The azimuthally Fourier-\/decomposed 3D Helmholtz equation}
\hypertarget{index}{}\label{index}\index{Example problem: The azimuthally Fourier-\/decomposed 3D Helmholtz equation@{Example problem: The azimuthally Fourier-\/decomposed 3D Helmholtz equation}}
In this document we discuss the finite-\/element-\/based solution of the Helmholtz equation in cylindrical polar coordinates, using a Fourier-\/decomposition of the solution in the azimuthal direction. This is useful for solving time-\/harmonic wave problems in 3D axisymmetric domains, \doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{e.\+g}. the scattering of acoustic sound field from a sphere, the example we consider below.
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_theory}{}\doxysection{\texorpdfstring{Theory\+: The Fourier-\/decomposed Helmholtz equation}{Theory: The Fourier-decomposed Helmholtz equation}}\label{index_theory}
...
If the equation is solved in an infinite domain (\doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{e.\+g}. in scattering problems) the solution must satisfy the so-\/called \href{http://en.wikipedia.org/wiki/Sommerfeld_radiation_condition}{\texttt{ Sommerfeld radiation condition}} which in 3D has the form  \[
...
\] so $\varphi \in [0,2\pi] $ remains unchanged, and ~\newline
 $\theta \in [0,\pi] $ sweeps from the north pole ( $ \theta = 0 $), via the equator ( $ \theta = \pi/2 $ ) to the south pole ( $ \theta = \pi $).\hypertarget{index_DtN}{}\doxysubsection{\texorpdfstring{The Dirichlet-\/to-\/\+Neumann mapping (\+Dt\+N)}{The Dirichlet-to-Neumann mapping (DtN)}}\label{index_DtN}
...
\] \doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{i.\+e}.  \[
\gamma(u(R,\theta))=
...
To facilitate the setup of the interaction between the {\ttfamily Fourier\+Decomposed\+Helmholtz\+Dt\+NBoundary\+Elements}, {\ttfamily oomph-\/lib} provides the class {\ttfamily Fourier\+Decomposed\+Helmholtz\+Dt\+NMesh} which provides storage for (the pointers to) the {\ttfamily Fourier\+Decomposed\+Helmholtz\+Dt\+NBoundary\+Elements} that discretise the artificial boundary. The member function {\ttfamily \doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{Fourier\+Decomposed\+Helmholtz\+Dt\+NMesh\+::setup\+\_\+gamma()}} pre-\/computes the $ \gamma $ values required for the imposition of equation (8). The radius $ R $ of the artificial boundary and the (finite) upper limit for the sum in (8) are specified as arguments to the constructor of the {\ttfamily Fourier\+Decomposed\+Helmholtz\+Dt\+NMesh}.

{\bfseries{NOTE\+:}} Since $ \gamma $ depends on the solution, it must be recomputed whenever the unknowns are updated during the Newton iteration. This is best done by adding a call to {\ttfamily \doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{Fourier\+Decomposed\+Helmholtz\+Dt\+NMesh\+::setup\+\_\+gamma()}} to {\ttfamily Problem\+::actions\+\_\+before\+\_\+newton\+\_\+convergence\+\_\+check()}. \mbox{[}If Helmholtz\textquotesingle{}s equation is solved in isolation (or within a coupled, but linear problem), Newton\textquotesingle{}s method will converge in one iteration. In such cases the unnecessary recomputation of $ \gamma $ after the one-\/and-\/only Newton iteration can be suppressed by setting {\ttfamily Problem\+::\+Problem\+\_\+is\+\_\+nonlinear} to {\ttfamily false}.\mbox{]}
...
\doxyfigcaption{Plot of the exact (green) and computed (red) real parts of the solution of the Fourier-\/decomposed Helmholtz equation for N=3 and a wavenumber of k\texorpdfstring{$^\wedge$}{\string^}2 = 10. }
\end{DoxyImage}
...
\doxyfigcaption{Plot of the exact (green) and computed (red) imaginary parts of the solution of the Fourier-\/decomposed Helmholtz equation for N=3 and a wavenumber of k\texorpdfstring{$^\wedge$}{\string^}2 = 10. }
\end{DoxyImage}
...
\DoxyCodeLine{\textcolor{comment}{}\ \mbox{\hyperlink{classAnnularQuadMesh}{Vector<double>}}\ \mbox{\hyperlink{namespaceProblemParameters_acb1788444ef78fe2adec824504f24246}{Coeff}}(N\_terms,1.0);}
\DoxyCodeLine{\textcolor{comment}{}}
...
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{FourierDecomposedHelmholtzProblem<QFourierDecomposedHelmholtzElement<3>}}\ >\ }
...
\DoxyCodeLine{\ \textcolor{keywordtype}{unsigned}\ \mbox{\hyperlink{classAnnularQuadMesh}{n\_r}}=10*\mbox{\hyperlink{namespaceProblemParameters_a23b618b9e3a0d282fd91aa3f3f7b9254}{ProblemParameters::El\_multiplier}};}
\DoxyCodeLine{\ }
\DoxyCodeLine{\ \textcolor{comment}{//\ \#\ of\ elements\ in\ theta-\/direction\ }}
\DoxyCodeLine{\ \textcolor{keywordtype}{unsigned}\ \mbox{\hyperlink{classAnnularQuadMesh}{n\_theta}}=10*\mbox{\hyperlink{namespaceProblemParameters_a23b618b9e3a0d282fd91aa3f3f7b9254}{ProblemParameters::El\_multiplier}};}
\DoxyCodeLine{\ }
\DoxyCodeLine{\ \textcolor{comment}{//\ Domain\ boundaries\ in\ theta-\/direction}}
\DoxyCodeLine{\ \textcolor{keywordtype}{double}\ \mbox{\hyperlink{classAnnularQuadMesh}{theta\_min}}=-\/90.0;}
\DoxyCodeLine{\ \textcolor{keywordtype}{double}\ \mbox{\hyperlink{classAnnularQuadMesh}{theta\_max}}=90.0;}
\DoxyCodeLine{\ }
...
\DoxyCodeLine{\ \textcolor{comment}{//-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/}}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{sprintf}}(\mbox{\hyperlink{classAnnularQuadMesh}{filename}},\textcolor{stringliteral}{"{}\%s/soln\%i.dat"{}},\mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.directory().c\_str(),}
...
\DoxyCodeLine{\ \textcolor{comment}{//-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/}}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{sprintf}}(\mbox{\hyperlink{classAnnularQuadMesh}{filename}},\textcolor{stringliteral}{"{}\%s/exact\_soln\%i.dat"{}},\mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.directory().c\_str(),}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.number());}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.open(\mbox{\hyperlink{classAnnularQuadMesh}{filename}});}
...
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{sprintf}}(\mbox{\hyperlink{classAnnularQuadMesh}{filename}},\textcolor{stringliteral}{"{}\%s/error\%i.dat"{}},\mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.directory().c\_str(),}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.number());}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.open(\mbox{\hyperlink{classAnnularQuadMesh}{filename}});}
...
\end{DoxyCodeInclude}
 The function {\ttfamily check\+\_\+gamma(...)} is used to check the computation of the $ \gamma $ integral. If computed correctly, its values (pre-\/computed at the Gauss points of the {\ttfamily Fourier\+Decomposed\+Helmholtz\+Flux\+Element}) ought to agree (well) with the derivative of the exact solution. They do; see \doxysectlink{index_acc}{The accuracy of the boundary condition elements}{3}.
...
\DoxyCodeLine{\ \textcolor{comment}{//-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/}}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{sprintf}}(\mbox{\hyperlink{classAnnularQuadMesh}{filename}},\textcolor{stringliteral}{"{}\%s/power\%i.dat"{}},\mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.directory().c\_str(),}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.number());}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.open(\mbox{\hyperlink{classAnnularQuadMesh}{filename}});}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{sprintf}}(\mbox{\hyperlink{classAnnularQuadMesh}{filename}},\textcolor{stringliteral}{"{}\%s/total\_power\%i.dat"{}},\mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.directory().c\_str(),}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{doc\_info}}.number());}
\DoxyCodeLine{}
\DoxyCodeLine{\ \textcolor{comment}{//\ Accumulate\ contribution\ from\ elements}}
\DoxyCodeLine{\ \textcolor{keywordtype}{double}\ \mbox{\hyperlink{classAnnularQuadMesh}{power}}=0.0;}
\DoxyCodeLine{\ \textcolor{keywordtype}{unsigned}\ \mbox{\hyperlink{classAnnularQuadMesh}{nn\_element}}=Helmholtz\_outer\_boundary\_mesh\_pt-\/>nelement();\ }
\DoxyCodeLine{\ \textcolor{keywordflow}{for}(\textcolor{keywordtype}{unsigned}\ \mbox{\hyperlink{classAnnularQuadMesh}{e}}=0;\mbox{\hyperlink{classAnnularQuadMesh}{e}}<\mbox{\hyperlink{classAnnularQuadMesh}{nn\_element}};\mbox{\hyperlink{classAnnularQuadMesh}{e}}++)}
\DoxyCodeLine{\ \ \{}
\DoxyCodeLine{\ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{FourierDecomposedHelmholtzBCElementBase<ELEMENT>}}\ *\mbox{\hyperlink{classAnnularQuadMesh}{el\_pt}}\ =\ }
\DoxyCodeLine{\ \ \ \ \textcolor{keyword}{dynamic\_cast<}\mbox{\hyperlink{classAnnularQuadMesh}{FourierDecomposedHelmholtzBCElementBase<ELEMENT>}}*\textcolor{keyword}{>}(}
\DoxyCodeLine{\ \ \ \ \ Helmholtz\_outer\_boundary\_mesh\_pt-\/>element\_pt(\mbox{\hyperlink{classAnnularQuadMesh}{e}}));\ }
\DoxyCodeLine{\ \ \ \mbox{\hyperlink{classAnnularQuadMesh}{power}}\ +=\ \mbox{\hyperlink{classAnnularQuadMesh}{el\_pt}}-\/>global\_power\_contribution(\mbox{\hyperlink{classAnnularQuadMesh}{some\_file}});}
\DoxyCodeLine{\ \ \}}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.close();}
\DoxyCodeLine{}
\DoxyCodeLine{\ \textcolor{comment}{//\ Output\ total\ power}}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{oomph\_info}}\ <<\ \textcolor{stringliteral}{"{}Radiated\ power:\ "{}}\ }
...
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ \ \ <<\ \mbox{\hyperlink{classAnnularQuadMesh}{power}}\ <<\ std::endl;}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.open(\mbox{\hyperlink{classAnnularQuadMesh}{filename}});\ }
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}\ <<\ \mbox{\hyperlink{namespaceProblemParameters_aaa674958a1ca6ee0b99de3377288c93f}{ProblemParameters::N\_fourier}}\ <<\ \textcolor{stringliteral}{"{}\ "{}}\ }
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ \ <<\ \mbox{\hyperlink{namespaceProblemParameters_aa5362de1af9e257fde4317c367158a93}{ProblemParameters::K\_squared}}\ <<\ \textcolor{stringliteral}{"{}\ "{}}\ }
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ \ <<\ \mbox{\hyperlink{classAnnularQuadMesh}{power}}\ <<\ std::endl;}
\DoxyCodeLine{\ \mbox{\hyperlink{classAnnularQuadMesh}{some\_file}}.close();}
\DoxyCodeLine{}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{std::complex<unsigned>\ \mbox{\hyperlink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{FourierDecomposedHelmholtzEquations<DIM>::u\_index\_fourier\_decomposed\_helmholtz}}()}
...
\begin{DoxyItemize}
\item The number of {\ttfamily Fourier\+Decomposed\+Helmholtz\+Dt\+NBoundary\+Element} along the artificial domain boundary. Since these elements are attached to the "{}bulk"{} {\ttfamily Fourier\+Decomposed\+Helmholtz\+Elements} it is important that the bulk mesh is sufficiently fine to resolve the relevant features of the solution throughout the domain.
...
Confirm that the (costly) re-\/computation of the $ \gamma $ integral in {\ttfamily actions\+\_\+before\+\_\+newton\+\_\+convergence\+\_\+check()} after the first (and only) linear solve in the Newton iteration can be avoided by declaring the problem to be linear.\hypertarget{index_acc_ex}{}\doxysubsubsection{\texorpdfstring{The accuracy of the boundary condition elements}{The accuracy of the boundary condition elements}}\label{index_acc_ex}

\begin{DoxyItemize}
\item Explore the accuracy (and computational cost) of the application of the DtN boundary condition by varying the number of terms included in the sum (8). Specifically, confirm that an obviously wrong result is obtained if we choose {\ttfamily \doxylink{namespaceProblemParameters_aa529b33b7feb959e0c044447bf0f6c6f}{Problem\+Parameters\+::\+Nterms\+\_\+for\+\_\+\+DtN}} \texorpdfstring{$<$}{<} {\ttfamily Problem\+Parameters\+::\+Nterms}. ~\newline
...
\doxyfigcaption{Plot of the exact (dashed) and computed (solid) gamma integral for N=3 and a wavenumber of k\texorpdfstring{$^\wedge$}{\string^}2 = 10. }
\end{DoxyImage}
\hypertarget{index_sphere}{}\doxysubsubsection{\texorpdfstring{Scattering of a planar acoustic wave off a sound-\/hard sphere}{Scattering of a planar acoustic wave off a sound-hard sphere}}\label{index_sphere}
...
\] \doxylink{classAnnularQuadMesh_acf01203e48fdee730b6c194bf538177c}{i.\+e}. the wave comprises a single azimuthal Fourier component with $
N=0$. Note that the \href{../../../../demo_drivers/fourier_decomposed_helmholtz/sphere_scattering/sphere_scattering.cc}{\texttt{ driver code}} already contains a namespace {\ttfamily \doxylink{namespacePlanarWave}{Planar\+Wave}} with several (but not all!) functions required for this task.
